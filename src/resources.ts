...
export function registerResources(server: McpServer): void {
  ...
  server.registerResource(
    ...
    "citation://panels",
    {
      title: "Saved panels",
      description:
        "List of saved query panels and per-panel snapshot counts. Mirrors track_queries action=list with snapshot context.",
      mimeType: "application/json",
    },
    async (uri) => {
  ...
  server.registerResource(
    ...
    "citation://docs/ai-crawlers",
    {
      ...
      description:
        "Quick reference for the AI crawlers `crawler_access_audit` checks, grouped by training vs real-time fetch.",
      mimeType: "text/markdown",
    },
    (uri) => ({
      contents: [
  ...
  server.registerResource(
    ...
    }),
    {
      ...
      description:
        "Cached citations for {domain}. Returns the most recent 200 entries. Same source as cited_for tool, surfaced as a resource for clients that subscribe.",
      mimeType: "application/json",
    },
    async (uri, variables) => {
