...
export function registerPrompts(server: McpServer): void {
  server.registerPrompt(
    "audit_citation_readiness",
    {
      ...
      description:
        "Score a URL's citation likelihood across public signals, then deep-validate its schema.org markup, then list the top fixes. Uses predict_citation + schema_audit.",
      argsSchema: {
  ...
  server.registerPrompt(
    "competitor_snapshot",
    {
      ...
      description:
        "Build a cross-engine competitor map for a query and compare your own URL to the top-cited competitors. Uses canonical_competitor_set + compete_for_query.",
      argsSchema: {
  ...
  server.registerPrompt(
    "ai_crawler_checkup",
    {
      ...
      description:
        "Verify that all major AI crawlers (GPTBot, ClaudeBot, PerplexityBot, CCBot, Google-Extended, etc.) can fetch a URL and aren't being blocked at robots.txt or the server. Uses crawler_access_audit.",
      argsSchema: {
  ...
  server.registerPrompt(
    "citation_gap_analysis",
    {
      ...
      description:
        "Surface queries where a domain ranks well on Google but isn't cited by AI engines - the closest editorial wins. Uses gsc_citation_gap.",
      argsSchema: {
  ...
  server.registerPrompt(
    "sitemap_coverage_review",
    {
      ...
      description:
        "Map a sitemap against the citation cache to see which URLs have been cited and which haven't. Identifies systemic visibility gaps. Uses sitemap_citation_map.",
      argsSchema: {
